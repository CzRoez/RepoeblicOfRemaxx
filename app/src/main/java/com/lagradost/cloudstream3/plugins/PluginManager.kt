...
object PluginManager {
    ...
    private val CLOUD_STREAM_FOLDER =
        Environment.getExternalStorageDirectory().absolutePath + "/CloudZtream®/"
    ...
    private fun getBrandedRepoName(repositoryUrl: String): String {
    return when (repositoryUrl.hashCode()) {
        -1089414092  -> "RpCpuX-VIP"
        567540747   -> "Phisher Repo"
        -2138799801 -> "RpCpuX-Repo"
        -484698041  -> "ExCloud Repo"
        -942783326  -> "IndoStream Repo"
        -815728861  -> "CNC Verse Repo"
        else -> extractRepoNameFromUrl(repositoryUrl)
    }
}
    ...
    val cleanRepoName = repositoryName
        ?: getBrandedRepoName(repositoryUrl)

    val folderName = getPluginSanitizedFileName(cleanRepoName)
...
fun extractRepoNameFromUrl(url: String): String {
    return try {
        url
            .substringAfterLast("/")      // plugins.json
            .substringBefore(".json")    // plugins
            .ifEmpty { "default_repo" }
    } catch (e: Exception) {
        "default_repo"
    }
}
